...
int32_t serial_start(stdin_reader_t reader, void* p_buffer, uint16_t u16_size)
{
    ...
    hserial.p_rxBuf     = p_buffer;
    hserial.u16_rxBufSz = u16_size;
...
static int32_t stdin_listen(void)
{
    ...
    if (0 < u32_inCnt)
    {
        hserial.reader(hserial.p_rxBuf, u32_inCnt);
        memset(hserial.p_rxBuf, 0x0, u32_inCnt);
    }

    /* start listening */
    s32_rc = p_usart->Receive(hserial.p_rxBuf, hserial.u16_rxBufSz);
